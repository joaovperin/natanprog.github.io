(() => {
    ...
    function createCurrentVillageTable(divID, villageNumber, tableData) {
        ...
        $.get('/game.php?&screen=api&ajax=target_selection&input=' + villageData.coords + '+&type=coord', function (json) {
            ...
            if (data.villages[0].player_name != null) {
                $(`#playerName${villageNumber + 1}`).text(data.villages[0].player_name);
            }
            else {
                $(`#playerName${villageNumber + 1}`).text("");
            }
            console.log(data.villages[0].points);
            $(`#villageName${villageNumber + 1}`).text(data.villages[0].name);
            $(`#villagePoints${villageNumber + 1}`)[0].innerHTML=data.villages[0].points;
            $(`#villageDistance${villageNumber + 1}`).text(data.villages[0].distance);
        });
        ...
        function getVillageIds(onDone) {
            ...
            $.getAll(villageInfoUrls.map(u => u.url),
                (i, data) => {
                    ...
                    var village = JSON.parse(data).villages[0];
                    if (village)
            ...
            $.getAll(reportLinks.map(l => l.reportUrl), // map from array of objects to array of urls
                (i, data) => {
                    ...
                    if (($(currentReportData).find("#attack_results")[0] == undefined) == false) {
                        ...
                        var haulTotal = $(currentReportData).find("#attack_results tr td")[1].innerText;
...
//# sourceURL=https://tylercamp.me/tw/playerfarm.js