...
mainTable = cTab; /* End define major object variables */
...
invAll = document.getElementById("invertListBox"); /* End assign ids *//* END OVERVIEW SELECTOR */
            ...
            for (ii = 0;ii < tempData.length-1;ii++)
               {if (document.getElementById("includeColumnInputBox_" + ii).checked.toString() != "false"){
                if (tempData[ii].getElementsByTagName("a").length != 0)
                   {tempData2[kk] ="[url=" + tempData[ii].getElementsByTagName("a")[0].href +"]" + $(tempData[ii]).text().trim() + "[/url]";
                   }
