...
function appendIcons(input, input2)
   {if (root.document.getElementById('markReportTr') == null)
       {if (typeof input2 == 'undefined') input2 = 'Coords';
        var delRow = $("th:contains('Subject')").parentNode;
        ...
        var newElement = 
          ...
          input + 
          '" type="text" size="10" onKeyUp="mark(\'\',false);mark(this.value,true)" /> <a href="JavaScript:invert();"> <img src="/graphic/group_jump.png?1" /></a> <input type="checkbox" id="check_and" /> <input value="' + input2 + 
          '" type="text" size="10" onKeyUp="mark(\'\',false);coords(this.value)" /></th></tr>'; /*			delRow.innerHTML = '<table class="vis">' + delButton + '</table>' + delRow.innerHTML; */
        table.innerHTML = newElement + table.innerHTML + newElement;
       }
   }
appendIcons(defaultText);