...
var scriptData = {
	...
	helpLink:
		'https://forum.tribalwars.net/index.php?threads/player-farms-finder-script.285245/',
};
...
function playerFarmsFinder() {
	...
	var [
		...
		barbsCount,
		playerVillagesCount,
	] = doPlayerBarbsCalculations(RADIUS);

	var scoutBarbsScript = generateScoutScript(barbsCoordsList);

	var content = `
		<p class="ra-fs12">
			<strong>Player:</strong> <a href="/game.php?screen=info_player&id=${playerId}" target="_blank" rel="noopener noreferrer">${playerName}</a><br>
			<strong>Player Villages:</strong> ${playerVillagesCount}
		</p>
		<p class="ra-fs12"><strong>Radius:</strong>
		...
		<p class="ra-fs12">
			<strong>Barbs found:</strong> <span id="barbsCount">${barbsCount}</span><br>
			<strong>Area:</strong> <span id="areaCoords">${areaCoords}</span>
		</p>
		<div class="ra-mb15">
		...
		<div class="ra-mb15">
			<label class="ra-fw600" for="barbCoordsScript">Scout Script:</label>
...
function doPlayerBarbsCalculations(radius) {
	...
	const [minX, minY, maxX, maxY] = getPlayerBoundaryCoords(
		playerVillages,
		radius
	);
	let areaCoords = minX + '|' + minY + ' - ' + maxX + '|' + maxY;

	var barbsInsidePlayerRadius = calculateBarbsInsideRadius(
		minX,
		minY,
		maxX,
		maxY
...
function updatePlayerBarbsList(select) {
	...
	// Filter barbarian villages by Player bounding coordinates
	var [barbsCoordsList, areaCoords, barbsCount] = doPlayerBarbsCalculations(
		radius
	);
