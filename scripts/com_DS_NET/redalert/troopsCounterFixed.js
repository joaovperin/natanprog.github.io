...
function fnExecuteScript() {
	try {
		...
		function fnTranslate(id) {
			...
			var lang = typeof (translation[win.game_data.market] == 'undefined')
				? 'en'
				: win.game_data.market;
		...
		function fnHasArchers() {
			return (
				parseInt(
					win.game_data.worldConfig.find('game archer').text() || '0',
					10
				) > 0
			);
		}
		function fnHasChurch() {
			return (
				parseInt(
					win.game_data.worldConfig.find('game church').text() || '0',
					10
				) > 0
			);
		}
		...
		function fnHasPaladin() {
			return (
				parseInt(
					win.game_data.worldConfig.find('game knight').text() || '0',
					10
				) > 0
			);
		}
		...
		function fnGetTroopCount() {
			...
			var gameVersion = parseFloat(
				win.game_data.version.split(' ')[1].replace('release_', '')
			...
			win.$(
				'#units_table tbody' + (gameVersion < 7.1 ? ':gt(0)' : '')
					...
					.each(function (cell, eleCell) {
						...
						if (cell % colCount) {
							...
							if (Math.floor(cell / colCount) != 1) {
								villageData.troops[
									(cell % colCount) - 1
								] += parseInt(win.$(eleCell).text() || '0', 10);
							}
						}
					});
	...
	*/
			...
			for (ii = 0; ii < villageTroops.length; ii++) {
				...
				win.$(win.game_data.unitConfig)
					...
					.each(function (i, e) {
						...
						total[unit].population +=
							...
							parseInt(win.$(e).find('pop').text(), 10);

						/* Defense */
						if (
							new RegExp('^(' + defense.join('|') + ')$').test(
								unit
							)
						) {
				...
				for (item in outputSummary) {
					if (outputSummary.hasOwnProperty(item)) {
						...
						for (
							jj = 0;
							jj < outputSummary[item].criteria.length;
							jj++
						) {
							...
							if (
								!(
									...
									!criteria.minpop ||
									total[criteria.unit].population >=
										criteria.minpop
								)
							) {
								isValid = false;
			...
			for (item in outputSummary) {
				if (outputSummary.hasOwnProperty(item)) {
					if (
						typeof groupSummary[outputSummary[item].group] ==
						'undefined'
					) {
	...
	*/
			...
			docSource += '\n';
			docSource +=
				'\t\t\t\tvar eleCoords = document.getElementById("coords_container");\n';
			docSource +=
				'\t\t\t\teleCoords.value = coords[id]?coords[id]:"";\n';
				...
				if (groupSummary.hasOwnProperty(item)) {
					...
					for (jj = 0; jj < groupSummary[item].length; jj++) {
						...
						docSource +=
							...
							"','" +
							fnTranslate(
								outputSummary[groupSummary[item][jj]].descID
							) +
							'\');" title="' +
							fnCriteriaToStr(
			...
			docSource +=
				'\t\t\t\t\t<tr><td style="padding:1em;"><textarea id="coords_container" style="width:100%;"></textarea></td></tr>\n';
			docSource += '\t\t\t\t</table>\n';
			docSource += '\t\t\t</table>\n';
			docSource += '\t\t</table>\n';
			docSource += '\t</body>\n';
		...
		if (!win.game_data.mode) {
			var vmode = win
				.$('#overview_menu td[class=selected] a')
				.attr('href')
				.match(/mode\=(\w*)/i);
			if (vmode) {
