javascript:
$.get(game_data.link_base_pure + "forum&screenmode=view_thread&" + "forum_id=" + forum_id + "&thread_id=" + thread_id, function(result) {


    if ($("#mobileHeader").is(":visible")) {
        var selector = ".post_text";
    } else {
        var selector = ".text";
    }


    var koordinatak = $(result).find(selector).text();

    (function() {

        var mainCoords = [koordinatak];
        var mainSlowestUnit = "kos";
        ...
        function doIt() {
            if (mainCoords.length > 0) {
                ...
                var now = new Date();
                var arr = new Date(now.valueOf() + Math.round(slowest() * distance(getCurrentCoord(), {
                    x: coord[0],
                    y: coord[1]
                })));
