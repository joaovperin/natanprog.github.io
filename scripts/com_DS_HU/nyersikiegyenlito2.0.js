/*************************************************
* Modified by IronFist
* 2016.08.07.
* Modified by öreg
* 2019.04.07.
* Use this script with quickbutton below
*************************************************/

//javascript:$.getScript("https://media.innogamescdn.com/com_DS_HU/scripts/nyersikiegyenlito.js");void 0; 

if (document.URL.match("screen=market&mode=send&village")) {
    window.close();
} else {
...
(function() {
	...
	function FillRes() {
		...
		function OKClick() {
			...
			if(doc.URL.match("confirm_send")) {
				formElement = $('form#market-confirm-form');
			} else {
				formElement = $('form[name="market"]');
			}
		...
		function insertValues() {
			var woodRegex = /wood=(\d+)/g;
			var clayRegex = /clay=(\d+)/g;
			var ironRegex = /iron=(\d+)/g;
			var woodRes   = woodRegex.exec(doc.URL);
			var clayRes   = clayRegex.exec(doc.URL);
			var ironRes   = ironRegex.exec(doc.URL);
			
			$('input[name="wood"]').first().val(woodRes[1]);
			$('input[name="stone"]').first().val(clayRes[1]);
			$('input[name="iron"]').first().val(ironRes[1]);
		}
		if (wood + clay + iron > 0) {
			OKClick()
                } else if (document.URL.match("wood=0&clay=0&iron=0")) {
                        window.close();
		} else {
			insertValues()
		}
