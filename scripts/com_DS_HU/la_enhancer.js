...
var magyaritas = "https://media.innogamescdn.com/com_DS_HU/scripts/la_enhancer/la_enhancer_hu.js";
...
window.top.$.getScript(scriptURL + 'lib/jstorage.js', function () {
    window.top.$.getScript(scriptURL + "resources.js", function () {
        ...
        window.top.$.getScript(magyaritas, function () {
            console.log("init");
            checkPage();
        });
    });
...
function checkWorking() {
    ...
    if (working == false && acknowledged == false) {
        buttons = [{text: 'OK', callback: null, confirm: true}];
        window.top.UI.ConfirmationBox("Hibát észleltünk ebben a verzióban. Ha nem tapasztaltál hibát, folytathatod a script tesztelését.", buttons, false, []);
...
function getNewVillage(way) {
    ...
    if (way == "n")
        window.top.UI.InfoMessage('Váltás a következő falura...', 500); else
        window.top.UI.InfoMessage('Váltás az előző falura...', 500);
...
function customSendUnits(link, target_village, template_id, button) {
    if (!checkIfNextVillage()) {
        ...
        window.top.$.post(window.top.Accountmanager.send_units_link, data, function (data) {
            if (data.error) {
                if (userset[s.next_village_units] && data.error == "Nem áll rendelkezésre elegendő egység") {
...
function applyFilters() {
    ...
    if (window.top.$('*:contains("Bot védelem")').length) {
        window.top.$('html, body').animate({scrollTop: (window.top.$('*:contains("Bot védelem")').offset().top - topContainer)}, 500);
        if (typeof showNotification === 'function') {
            showNotification('custom', ['Az LA Enhancer bot védelmet észlelt. Kérlek reagálj rá, hogy folytatni tudd a farmolást.'], null, 'Bot védelem');
        }
        cansend = false;
    } else {
    ...
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            ...
            if (button.hasClass("farm_icon_disabled") || button.html() == undefined) {
                window.top.UI.ErrorMessage("Ez a gomb nem használható. Sor kihagyása...", 1000);
                button.closest('tr').hide();
            }
            else {
                button.click();
...
function editKey(e) {
    if ((e.keyCode <= 37 && e.keyCode >= 40) || (e.keyCode <= 48 && e.keyCode >= 90)) {
        window.top.UI.ErrorMessage("Csak betűk, számok és nyilak használhatóak. Kérlek próbálj meg egy másik gombot.", 2500);
    } else {
        ...
        window.top.UI.SuccessMessage(keyToChar + " -t hozzárendeltem a(z)  " + keyToEdit + " gombhoz.");
...
function setKeyEditMode(n) {
    editingKey = true;
    keyToEdit = n;
    window.top.UI.InfoMessage("Nyomj meg egy számot, betűt vagy nyilat, hogy beállítsd gyorsgombnak az <span style='font-weight:bold;'>" + n + "</span> gombhoz.", 2500);
    return false;
}
...
function setDefaultLanguage() {
    ...
    switch (domain) {
        default:
            window.top.$.jStorage.set("language", "hu");
            break;
    }
}
