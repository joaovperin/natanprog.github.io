    ...
    if (!TwCheese.hasTool(toolId)) {
        ...
        function suggestRedirect(options) {
            ...
            if (skippableId && userConfig.get(skipKey(skippableId), false)) {
                window.UI.InfoMessage(`Átirányítás a <strong>${screenName}</strong>-re...`);
                setTimeout(() => window.TribalWars.redirect(screen, uriParams), 200);        
                return;
            }    
        
            let buttonConfirm = {
                text: 'A gyűjtögetéshez',
                callback: () => {
                    if (skippableId) {
        ...
        function buildContent(message, options) {
            ...
            if (options.skippableId) {
                html += `<div class="twcheese-suggest-redirect-skip-container">
                    ...
                    <span>Legközelebb ne kérdezze meg, csak navigáljon oda.</span>
                </div>`
            }
            return html;
        }
        ...
        class ScavengePreferencesWidget_ScavengePreferencesWidget extends AbstractWidget["a" /* AbstractWidget */] {
                        ...
                        <tr>
                            <td>
                                <label>
                                    ...
                                    Először a legjobb lehetőségeket használja.
                                    <br/><span class="hint">(ajánlott, ha nem leszel gépnél)</span>
                                </label>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <label>
                                    ...
                                    Ugyanarra az időtartamra az összes lehetőségnél.
                                    <br/><span class="hint">(ajánlott, ha bármikor azonnal újra tudod küldeni)</span>
                                </label>    
                            </td>
                        </tr>
                    </table>
            ...
            createOptionsSectionHtml() {
                ...
                return `
                    <table class="vis options-section">
                        <tr><th colspan="2">Használható lehetőségek</th></tr>
            ...
            createOptionRowHtml(option) {
                ...
                return `<tr>
                    ...
                    <td>(${option.getLootPercent()}% teherbíró képesség)</td>
                </tr>`;
            }
        
            createTroopsSectionHtml() {
                let helpTooltip = [
                    `A lefoglalt csapatok nem lesznek elküldve gyűjtögetni.`,
                    `<br/>`,
                    `<br/>Példa:`,
                    ` :: Van 500 lándzsásod és 100 foglalt. Legfeljebb 400 lándzsást lehet elküldeni gyűjtögetni.`
                ].join('');
                ...
                return `
                    <table class="vis troops-section">
                        <tr>
                            <th>Használat</th>
                            <th>Lefoglalt <img src="${ImageSrc["a" /* ImageSrc */].info}" title="${helpTooltip}" class="tooltip info"></th>
                        </tr>
            ...
            createTroopsOrderSectionHtml() {
                let helpTooltip = [
                    `A felső csoportokban lévő egységek az alsóbb csoportokban lévők előtt lesznek elküldve.`,
                    `<br/>`,
                    `<br/>Ugyanazon csoport csapatai arányosan lesznek szétosztva.`
                ].join('');
        
                return `
                    <table class="vis troop-order-section">
                        <tr>
                            <th>Sorrend <img src="${ImageSrc["a" /* ImageSrc */].info}" title="${helpTooltip}" class="tooltip info"></th>
                        </tr>
        ...
        function suggestRedirectToScavengeScreen() {
            suggestRedirect({
                message: 'A használathoz a gyűjtögető felületen kell futtatnod.',
                screen: 'place',
                screenName: 'Gyűjtögető felület',
                uriParams: {
                    mode: 'scavenge'
                },
        ...
        function insertPreferencesLauncher() {
            let $launcher = $(`<a href="#">&raquo; beállítások</a>`)
                .css({
                    fontSize: 'small',
                    marginLeft: '10px'
                })
        ...
        function insertNarcissim() {
            let $narcissism = $(`<span>Készítette: <a href="https://forum.tribalwars.net/index.php?members/28484">cheesasaurus</a></span>`)
                .css({
                    float: 'right',
                    fontSize: 'xx-small',
                    fontWeight: 'normal'
        ...
        function prepareBestOption(informUserOfIssues = true) {
            ...
            if (usableOptionIds.length < 1) {
                if (informUserOfIssues) {
                    window.UI.ErrorMessage(`Nincs használható opció a gyűjtögetéshez`);
                }        
                return;
            }
            ...
            if (!troopsAssigner.areTroopsSufficient(availableTroops)) {
                if (informUserOfIssues) {
                    window.UI.ErrorMessage(`Nincs elég egység a gyűjtögetéshez`);
                }
                return;
            }
