...
licznik_wojska.nazwyJednostek = "lándzsás,kardforgató,bárdos,íjász,kém,könnyűlovas,lovasíjász,nehézlovas,faltörő kos,katapult,lovag,nemes".split(",");
var okienko = "<h2 align='center'>Sereg számláló</h2><p align='right'><small>- v1.1 by <font face color='red'><b>bArNNNi</b></font></small></p><table width='100%'><tr><th>Csoportok: <select id='listaGrup' onchange=\"licznik_wojska.link = this.value; pobierzDane();\"><option value='"+licznik_wojska.link+"'>Összes</select>";
okienko += "<tr><td><table width='100%'><tr><th colspan='4'>Szűrés: <select onchange=\"zmiana(this.value);\"><option value='0'>Rendelkezésre álló<option value='0p2p3'>Összes csapat<option value='1'>A faluban lévők<option value='1m0'>Csak a támogatás<option value='2'>Támogatásban<option value='3'>Úton</select><tbody id='dostepne_wojska'></table><tr><th><b id='ilosc_wiosek'></b><a href='#' style='float: right;' onclick=\"eksportuj();\">Exportálás</a></table>";
...
function pobierzDane(){
    ...
    function processResponse(){
        if (r.readyState == 4 && r.status == 200) {
            ...
            if(!tabela){ $("#dostepne_wojska").html("Nincs falu a csoportban. <br>Válassz más csoportot!"); $("#ilosc_wiosek").html(" Üres"); return false;}
            var grupy = $(requestedBody).find('.vis_item').get()[0].getElementsByTagName(mobile?'option':'a');
            if(tabela.rows.length>4000) alert("Figyelem!\nCsak az első 1000 falut vizsgálom!");
            if(!licznik_wojska.pobraneGrupy){
                for(i=0;i<grupy.length;i++){
                    nazwa = grupy[i].textContent;
                    if(mobile && grupy[i].textContent=="minden") continue;
...
function wypisz(sumaWojskDoWypisania){
    ...
    for(i=0;i<licznik_wojska.obrazki.length;i++){
        ...
        elem += (i%2==0?"<tr>":"")+"<th width='20'><a href='https://help.klanhaboru.hu/wiki/"+licznik_wojska.nazwyJednostek[i]+"' target='_blank'><img src='"+image_base + "unit/unit_"+licznik_wojska.obrazki[i]+".png'></a><td bgcolor='#fff5da'>"+sumaWojskDoWypisania[i];
    }
    licznik_wojska.eksport += "</textarea>";
    $("#dostepne_wojska").html(elem);
    $(mobile?'#loading':'#loading_content').hide();
    $("#ilosc_wiosek").html("Összesen "+((tabela.rows.length-1)/5)+" falu vizsgálva!");
}