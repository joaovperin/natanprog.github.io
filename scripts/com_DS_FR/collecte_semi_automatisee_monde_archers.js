...
var archers = document.getElementsByClassName("units-entry-all")[3].innerHTML;
archers = archers.substr(1, archers.length - 2);
archers = Math.max(archers - archer,0);

var lightC = document.getElementsByClassName("units-entry-all")[4].innerHTML;
lightC = lightC.substr(1, lightC.length - 2);
lightC = Math.max(lightC - leger,0);

var mArchers = document.getElementsByClassName("units-entry-all")[5].innerHTML;
mArchers = mArchers.substr(1, mArchers.length - 2);
mArchers = Math.max(mArchers - marcher,0);

var heavyC = document.getElementsByClassName("units-entry-all")[6].innerHTML;
...
scavengeOptions = {
    ...
    'Grosse collecte': [{
        ...
        {
            type: 'archer',
            count: archers / (12 / 13) * (3 / 26)
        },
        {
            type: 'marcher',
            count: mArchers / (12 / 13) * (3 / 26)
        },
        {
            type: 'heavy',
    ...
    'Collecte moyenne': [{
        ...
        {
            type: 'archer',
            count: archers / (21 / 26) * (3 / 13)
        },
        {
            type: 'marcher',
            count: mArchers / (21 / 26) * (3 / 13)
        },
        {
            type: 'light',
...
function findNextButton(option) {
    let btn = $(`.scavenge-option:contains("${option}")`).find('a:contains("Lancer")');
