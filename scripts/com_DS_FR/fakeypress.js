...
function run() {
    ...
    if (checkCookie()) {
        if ($.cookie(cookieName)
            .indexOf('{') == -1) {
            alert("Essayer de changer le nom de la variable cookieName. Si le problÃ¨me persiste, rendez vous sur le forum.");
            ...
            UI.ErrorMessage("A cause d'une mise a jour, les donnÃƒÂ©es ont ÃƒÂ©tÃƒÂ©s perdues. Veuillez recommencer les assignations de touches.", 5000);
...
function addTable() {
    ...
    $('#divFAPress')
        .append($(
            "<table id='faKeySettings' class='vis' style='width:100%' cellspacing='0'><thead><tr><th colspan='3'><em>Paramètres</em> - <a href'#' id='showSettings' onclick='return doSettings()'>Cacher</a></thead><tbody id='bodySettings'><tr><td colspan='1' align='center'><input type='checkbox' id='chbLoadPages' onclick='return chkBoxClick($(this).is(\":checked\"), " +
            ...
            pos.s.remred +
            ")'> <img src='https://media.innogamescdn.com/com_DS_FR/Scripts/Pillage/red.png' title='Défait' alt='' class='tooltip' /> Défait</tr><tr><td align='right' colspan='2'><input type='button' class='btn' id='btnSettingsReset' value='Reset' onclick='resetCookie(); UI.SuccessMessage(\"Reset effectuÃƒÂ©\",1000); run(); return false;'><input type='button' class='btn' id='btnSettingsApply' value='Appliquer' onclick='saveSettings(); run(); return false'><input type='button' class='btn' id='btnSettingsSave' value='Sauvegarder' onclick='saveSettings(); return false;'></tr></tbody></table>"
        ));
...
function saveSettings() {
    ...
    userset[pos.s.MaxNbAttacks] = $('#txtNbAttacks')
        .val();
    setCookie(cookieName, "{" + version + "}[" + userkeys.toString() + "][" + userset.toString() + "]", 180);
    UI.SuccessMessage("ParamÃƒÂ¨tres sauvÃƒÂ©s", 1000);
}
        ...
        $.ajax({
            ...
            success: function(data) {
				...
				*/
                ...
                //On change manuellement le village dans les donnÃ©es car ce n'est pas fait automatiquement
...
function openLoader() {
    ...
    setTimeout(function() {
        if (incrementalSwitchPage <= currentIncremental) {
            UI.ErrorMessage('Un problÃƒÂ¨me a ÃƒÂ©tÃƒÂ© rencontrÃƒÂ©, la page va se recharger.');
            var currentURL = document.URL.split("village=") + "village=" + game_data.village.id + "&screen=am_farm";
            window.location.href = currentURL;
			console.log("La page a mis trop de temps Ãƒ  se charger.")
        }
    }, 4000);
