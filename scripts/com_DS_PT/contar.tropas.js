...
try{
  ...
  */
		fnDebugLog("");
		fnDebugLog(""+(win.game_data.isPremium?"":""));
		fnDebugLog(""+(fnHasChurch()?"":""));
		fnDebugLog(""+(fnHasPaladin()?"":""));
		fnDebugLog(""+(fnHasArchers()?"":""));
		fnDebugLog(""+(fnHasMilitia()?"":""));
		fnDebugLog(""+(win.location.href.match(/t\=\d+/i)?"":""));
...
*/	
		...
		docSource += "<html>\n";
		docSource += "\t<head>\n";
		docSource += "\t\t<title>SumÃ¡rio das Tropas</title>\n";
...
*/
		...
		docSource += "\t\t\t\t\t\t\t\t<table class=\"vis\" width=\"100%\">\n";
		docSource += "\t\t\t\t\t\t\t\t\t<tr><th colspan=\"2\" style=\"white-space:nowrap;\">" + fnTranslate(curGroup++) + "</th></tr>\n";
		docSource += "\t\t\t\t\t\t\t\t\t<tr class=\"" + "row_a" + "\"><td><span>Contigente:</span></td><td style=\"white-space:nowrap;\"><span> " + summary.unitTotal.tally + "</span></td></tr>\n";
		docSource += "\t\t\t\t\t\t\t\t\t<tr class=\"" + "row_b" + "\"><td><span>População Total:</span></td><td style=\"white-space:nowrap;\"><span> " + summary.unitTotal.population + "</span></td></tr>\n";
		docSource += "\t\t\t\t\t\t\t\t</table>\n";
		...
		docSource += "\t\t</table>\n";					
		docSource += "\t</body>\n";
		docSource += "\t<center>Script compativél com a versão 8.2<br/><br>Última verificação do script efetuada dia 25º/03/2012</body>\n";
		docSource += "</html>\n";

		var popup=win.open('about:blank','','width=460,height=770,scrollbars=yes');
	...
	if(!win.game_data.mode){
		var vmode='Execute o script em \n\nVISUALIZAÇÕES» TROPAS » TODOS\n[ Na opção: Todos | Próprias | Na Aldeia | Fora | Em trânsito | Defesa | Apoios ]';
		if(vmode){
