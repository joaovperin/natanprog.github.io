/* 
	...
	NOTE	:
		* Use the following Client-side script launcher
		...
		javascript:(window.main||self).$.getScript('http://dl.dropbox.com/u/25377948/twscripts/sort_rh.js',function(){fnOverviewSorter()});void(0);
...
OverviewSorter.prototype.__defineGetter__('overviewConfig',function(){
	var hasOriginVillage=(!!this._win.$('#incomings_table tr th a[href*="start_village_name"]').length);
...
*/
			'units':{
				...
				'cols':[
					...
					{name:'knight',       regex:/(\d+)/,                                       desc:[['number']],                                               visible:this.hasPaladin},
					{name:'snob',         regex:/(\d+)/,                                       desc:[['number']],                                               visible:true},
					{name:'militia',      regex:/(\d+)/,                                       desc:[['number']],                                               visible:this.hasMinVersion(7.0)&&this.hasMilitia&&(['type=own_home','type=there'].indexOf(this._win.$.trim(this._win.location.href.match(/type\=\w+/i)))>=0)},
...
//***** NOTE: Some worlds now contain origin village info
					{name:'origin',       regex:/(.*?)\s\((\d+)\|(\d+)\)\sK(\d+)/,             desc:[['text'],['number',' ('],['number','|'],['number',') K']], visible:this.hasMinVersion(7.0)&&hasOriginVillage},
//*******************************************************
					{name:'player',       regex:/([.\s\S\w\W\d\D]*)/,                          desc:[['text']],                                                 visible:true},
					{name:'arrival time', regex:/([.\s\S\w\W\d\D]*)/,                          desc:[['text']],                                                 visible:true},
					{name:'arrival in',   regex:/([.\s\S\w\W\d\D]*)/,                          desc:[['text']],                                                 visible:true},
					//{name:'req support',  regex:null,                                          desc:[],                                                         visible:!this._win.location.href.match(/subtype\=supports/i)}
					{name:'req support',  regex:null,                                          desc:[],                                                         visible:(this._win.$('#incomings_table tr th').length>7)}
				]
			},
			'buildings':{
			...
			'tech':{
				...
				'cols':[
					...
					{name:'flagge',       regex:null,                                          desc:[],                                                         visible:$('#techs_table tr th').length>15}
				]
...
OverviewSorter.prototype.setup=function(){
	...
	/* Overviews->Troops has a more complex layout, so restrict it */
	var overview = (this.isPremium?this._win.game_data.mode:'non-premium');
	if(overview=='units'){
		var unitsType=this._win.location.href.match(/type\=(\w+)/i);
		if(!(unitsType&&(['own_home','there','away','moving'].indexOf(unitsType[1])>=0))){
			throw('The script can not be run on this Screen');
		}
	}
...
OverviewSorter.prototype.onClick=function(myself,sender){
	...
	myself.debug('DEBUG: Number of rows to sort='+myself._valueArray.length);

	/* Clear any highlighted sorting arrows */
	myself.debug('DEBUG: Clearing highlighted sorting arrows...');
	...
	myself.debug('DEBUG: '+['fieldName='+fieldName,'fieldIndex='+fieldIndex,'sortDir='+(sortDir<0?'descending':'ascending'),'fieldType='+fieldType].join(', '));
	
	function fnAdjustNumber(value){return parseInt(value.toString().replace('.','','g')||'0',10);}

	/* Sort the active column */
	myself.debug('DEBUG: Sorting...');
	...
	myself.debug('DEBUG: Clearing old table rows...');
	myself._win.$(myself.activeOverview.table + ' tr:gt(0):lt('+myself._valueArray.length+')').remove();
	
	/* Insert the Sorted Table Rows */
	myself.debug('DEBUG: Inserting sorted table rows...');
	// ***** BREAKING on large accounts in Firefox
	...
	// *****
	var srcHTML='';
	myself._valueArray.forEach(function(e,i){
		srcHTML+=e[0];
	});
	myself._win.$(srcHTML).insertAfter(myself._win.$(myself.activeOverview.table + ' tr:eq(0)'));
	// *****

	myself.debug('Done sorting.<br/>');
	return false;
...
OverviewSorter.prototype.execute=function(){
	...
	try{
		// Check Permissions.
		if(myself._win.game_data.world=='zz2'){
			if([16467].indexOf(parseInt(myself._win.game_data.player.id,10))<0){
				alert('Hi '+myself._win.game_data.player.name+'!\n\nYour scripts have been disabled by dalesmckay\nSend him a mail if you wish to help with testing');
				return false;
			}
		}
		...
		myself.print('<br/>execution completed<br/>',this._debugID + '_done');
	}
	catch(objError){
		this.handleError(objError);
	}
