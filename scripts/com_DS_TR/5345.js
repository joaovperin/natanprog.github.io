...
{	
	...
	/*==== title bar ====*/				
		...
		titleBar.rows[0].insertCell(-1);
		titleBar.rows[0].cells[0].innerHTML = '<b>Köy isimleri</b> (Maksimum 32 karakter)';
		...
		narcismElement.innerHTML = '</b>Yapimci Cheesasaurus, Türkçe Çeviri Trafik</b>';
		narcismElement.style.fontSize = '10px';
		...
		var options = [
			{
				...
				description:'Numarali isimlendirme',
				defaultLabel:'0',
				startNum:'0',
				digits:'4',
				example:'',
			...
			{
				name:'continent',
				description:'Kita numarasi ekleme',
				defaultLabel:' K',
				example:'55',
				enabled: true
			},
			{
				name:'insert_text0',
				description:'Belirtilen köy ismi',
				defaultLabel:'Köy ismi',
				example:'',
				enabled: true
			},	
			{
				name:'distance',
				description:'Mesafeye göre isimlendirme (Yazilan kordinata göre)',
			...
			{
				name:'sector',
				description:'Bölgeye göre isimlendirme',
				defaultLabel:' ',
				example:'55:12:2',
				enabled: false
			},
			{
				name:'direction',
				description:'Kita konumuna göre isimlendirme (Kuzey - K vs.)',
				defaultLabel:' ',
				example:'NE',
				enabled: false
			},
			{
				name:'random_text',
				description:'Rastgele isimlendirme',
				defaultLabel:' ',
				example:'Trafik',
				enabled: false
			},			
	...
	content.saveConfig = function()
	{
		...
		UI.InfoMessage('Ayarlar Kaydedildi',3000,'success');
	};
	...
	{
		if(document.getElementById('twcheese_name_preview').innerHTML.length >= 33)
		{
			UI.InfoMessage('isim fazla uzun (Maksimum 32 karakter)',5000,'error');
		}				
		else
		{
		...
		for(var i=0;i<options.length;i++)
		{
			...
			if(options[i].name == 'number_villages') //put the special numbering input in the label spot
			{
				...
				numberingInputTable.rows[0].cells[0].innerHTML = 'ilk sayi';
				numberingInputTable.rows[0].cells[0].style.width = '80px';
				numberingInputTable.rows[0].cells[1].innerHTML = 'Basamak';
		...
		modeForm.id = 'twcheese_name_mode_form';
			...
			modeForm.innerHTML += 'Kullanilan isim silinsin';
			...
			modeForm.innerHTML += 'Arkasina eklensin';			
			...
			modeForm.innerHTML += 'Önüne eklensin';
		...
		buttonDiv.style.padding = '10px';
			...
			saveButton.innerHTML = 'Ayarlari kaydet';
			buttonDiv.appendChild(saveButton);
			
			/*==== confirm button ====*/
			var confirmButton = document.createElement('a');
			confirmButton.className = 'btn-default btn-green';
			confirmButton.innerHTML = 'Tamamla';
		...
		$('.quickedit-vn').each(function(key,village){ //each village
			...
			for(var j=0; j<config.length; j++) //each configuration option
			{
				if(config[j].enabled)
				{
					...
					if (config[j].name == 'direction') {
						var directionNames = [['KB','K','KD'],['B','M','D'],['GB','G','GD']];
			...
			if (name.length <= 32) {
				...
				$(village).find('input[type=button]').click();*/
							...
							//	İşlem sınırını aşma sorunu giderilmiştir (Konsantre)
				$('#content_value').find('.rename-icon').each(function(i,that){setTimeout(function(){
				$(that).click();$(village).find('input[type=text]').val(name);$(village).find('input[type=button]').click();},200*i)});
			}
			else
				UI.InfoMessage(' isim fazla uzun (Maksimum 32 karakter).<br/>Yeni isim bütün köylerde uygulanamadi.',5000,'error');
...
/*==== main ====*/
	...
	if (game_data.screen == 'overview_villages' || canNameVillages) {
		...
		UI.InfoMessage('Genel bakis icerisinde kullanin.',5000,'error');
	}