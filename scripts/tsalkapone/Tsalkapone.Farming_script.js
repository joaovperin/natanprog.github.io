 ...
 var tsalkapone_trans = (function() {
    ...
    tsalkapone_trans.english= { 
	 ...
	 notification:"Notification from",		
		...
		fake_lang:"Language",
		info1:"The script will only use one infatry-type or cavalry-type unit, based on a priority-order depending on each unit's speed, to insert in the rally point. If the available units of a unit-type are equal to zero the script will skip this unit-type. If it is above zero but below the selected value every remaining amount of the unit will be inserted. The script will first use light cavalry, next heavy, next marcher and so on.<br> The scouts, rams and catapults are exceptions to this rule and their values will be inserted regardless",
		...
		info3:"If the value of this variable is <b><font color='blue'>true</b></font>, swords and spears will be inserted together. In any other case they will be inserted separately.",
		info4:"Enter a value that will be used as the name of the script",
		info5:"Enter a value that will be used as cookie name for the coords' order",
		info6:"Enter the coordinates of the desired targets. Add as many as you like separating them with space i.e.",
		info7:"Enter the desired values for each unit",
		info8:"The language of every rally point's script that doesn't display a settings menu is defined by the language selection in the script",
	},
	notes:{
...
tsalkapone_trans.greek= { 
	 ...
	 notification:"Ειδοποίηση από τον",
		...
		info1:"Το script θα επιλέξει μόνο ένα είδος μονάδας για να εισάγει στο Μέρος Συγκέντρωσης. Η επιλογή βασίζεται σε μια σειρά προτεραιότητας βάση αύξουσας ταχύτητας ξεκινώντας από ελαφρύ και καταλήγοντας σε ξίφος. Αν η τιμή μιας μονάδας εντός του μέρους συγκέντρωσης είναι ίση με το μηδέν τότε θα αγνοηθεί από το script. Αν είναι μικρότερη της επιλεγμένης θα εισαχθούν όλες οι εναπομείνατες μονάδες. Οι ανιχνευτές, κριοί και καταπέλτες αποτελούν εξαίρεση του κανόνα και η τιμή τους εισάγεται ανεξάρτητα από τις υπόλοιπες τιμές",
		...
		info3:"Αν η τιμή της συγκεκριμένης μεταβλητής είναι ίση με <b><font color='blue'>true</b></font>, τότε οι τιμές των δορατοφόρων και των ξιφομάχων θα εισαχθούν μαζί. Σε κάθε άλλη περίπτωση θα εισαχθούν ξεχωριστά.",
		info4:"Εισάγετε μια τιμή που θα χρησιμοποιηθεί ως ονομασία του script",
		info5:"Εισάγετε μια τιμή που θα χρησιμοποιηθεί ως ονομασία cookie για τη σειρά των συντεταγμένων",
		info6:"Εισάγετε τις συντεταγμένες των στόχων προσθέτοντας όσες επιθυμείτε διαχωρίζοντας τες με κενό π.χ.",
		info7:"Ορίστε τις επιθυμητές τιμές για κάθε μονάδα",
		info8:"Η γλώσσα κάθε script που λειτουργεί από το Μέρος Συγκέντρωσης και δεν διαθέτει ενσωματωμένο μενού ρυθμίσεων καθορίζεται από την επιλογή γλώσσας στο script",
	},
	notes:{
...
'<font color="red"><b>Tsalkapone_secure_attack:</b></font> '+tsalkapone_trans.general.info3+'.'+
'<br><br><font color="red"><b>Tsalkapone_script_title:</b></font> '+tsalkapone_trans.general.info4+'.'+
'<br><br><font color="red"><b>Tsalkapone_cookieID:</b></font> '+tsalkapone_trans.general.info5+'.'+
'<br><br><font color="red"><b>Tsalkapone_coords:</b></font> '+tsalkapone_trans.general.info6+' <i>532|516 451|450 430|420</i>.'+
'<br><br><font color="red"><b>spy,spear,axe,light,ram etc:</b></font> '+tsalkapone_trans.general.info7+'.'+
'<p><hr><font color=maroon><b><u>4. '+tsalkapone_trans.general.fake_lang+'</u></b></font></p>' +
'<p>'+tsalkapone_trans.general.info8+' <b><font color="blue">Commander Script</font></b>.</p>'+
...
function tsalkapone_insert_troops1() {   
 ...
 else if (heavy1 >= βαρύ && βαρύ>0) {   d.forms[0].heavy.value= βαρύ; }
else if (heavy1 < βαρύ && heavy1>0 && βαρύ>0) {   d.forms[0].heavy.value= heavy; } 
         else if (document.getElementById('unit_input_archer') && marcher1 >= έφιππος && έφιππος>0) {   d.forms[0].marcher.value= έφιππος; }
else if (document.getElementById('unit_input_archer') && marcher1 < έφιππος && marcher1>0 && έφιππος>0) {   d.forms[0].marcher.value= marcher1; }   
         
else if (axe1 >= τσεκούρι && τσεκούρι>0) {   d.forms[0].axe.value= τσεκούρι; }
else if (axe1 < τσεκούρι && axe1>0 && τσεκούρι>0) {   d.forms[0].axe.value= axe1; }
else if (Tsalkapone_secure_attack === true && spear1>=δόρυ && δόρυ>0 && sword1>= ξίφος && ξίφος>0)
{d.forms[0].spear.value= δόρυ; d.forms[0].sword.value= ξίφος;}
else if (Tsalkapone_secure_attack === true && spear1< δόρυ && spear1>0 && δόρυ>0 && sword1< ξίφος && sword1>0 && ξίφος>0)
{d.forms[0].spear.value= spear1; d.forms[0].sword.value= sword1;}
	else if (spear1 >= δόρυ && δόρυ>0) {   d.forms[0].spear.value= δόρυ; }
else if (spear1 < δόρυ && spear1>0 && δόρυ>0) {   d.forms[0].spear.value= spear1; } 
         
else if (document.getElementById('unit_input_archer') && archer1 >= τοξότης && τοξότης>0) {   d.forms[0].archer.value= τοξότης; }
else if (document.getElementById('unit_input_archer') && archer1 < τοξότης && archer1>0 && τοξότης>0) {   d.forms[0].archer.value= archer1; } 

    else if (sword1 >= ξίφος && ξίφος>0) {   d.forms[0].sword.value= ξίφος; }
