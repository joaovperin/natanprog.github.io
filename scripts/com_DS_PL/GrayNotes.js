    ...
    let NotesScript = {
        ...
        get_context: function () {
            ...
            if (NotesScript.context.side !== 'att' && NotesScript.context.side !== 'def') {
                let user_answer = prompt('W raporcie jeste\u{15B} agresorem (wpisz \'off\') czy obrońcą (wpisz \'deff\')?');
                if (user_answer !== null && (user_answer.trim() === 'off')) {
                    NotesScript.context.side = 'att';
                } 
                else if (user_answer !== null && (user_answer.trim() === 'deff')) {
                    NotesScript.context.side = 'def';
                } else {
        ...
        generate_attack_info: function (attack_info) {
            ...
            if (attack_info.attack_results) {
                if (attack_info.attack_results.ram_result) {
                    ...
                    properties.push(`Mur uszkodzony z poziomu ${from} do poziomu ${to}`)
                }
                if (attack_info.attack_results.catapult_result) {
                    ...
                    properties.push(`${target} uszkodzono z poziomu ${from} do poziomu ${to}`);
                }
            }
        ...
        parse_notebook: function (old_notes) {
            ...
            let new_note = `[size=15][b][color=#0000ff]${NotesScript.generate_village_info()}[/color][/b][/size] \n\n${attack_infos_text}\n___${user_notes}`;
            return new_note;
        },
        ...
        get_old_village_info: function (old_notes) {
            ...
            let sim_match = village_info_properties_text.find(x => x.startsWith('Symulacja'));
            if (sim_match) {
