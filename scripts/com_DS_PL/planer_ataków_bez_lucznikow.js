...
function wypiszMozliwosci(){
	...
	$('#lista_wojska tbody').html(html.join("\n")+(ilosc_wiosek?"<tr><td id='export_bb' colspan="+(dane.predkosci.length+4)+"><a href='#' onclick=\"$('#export_bb').html('<textarea cols=100 rows=2 onclick=\\'this.select()\\'>[table][**]Jednostka[||]èrÛd≥o[||]Cel[||]Czas wyjúcia[||]Rozkaz[/**]\\n'+tabelkaBB.join('')+'[/table]</textarea>');\" ><img src='"+image_base+"igm/export.png' > Eksportuj rozpiskÍ</a>":''));
...
function wybieranieWiosek(){
	...
	okienko = "<tr><th style=\"cursor:pointer;\" onclick=\"ustaw_min(0); $('#wyborWojsk tr:has(td)').each(function(i){$(this).show();}); \">Minimalna\u00A0iloúÊ\u00A0wojsk:";
	for(i=0;i<dane.predkosci.length;i++)
...
function rysujPlaner(){
	...
	elem += "<th>Czas\u00A0wyjúcia<th><span class=\'icon header time\'><th><b>Rozkaz</b></thead>";
